...
async function fetchExchangeRates() {
    try {
        ...
        document.getElementById('exchange-update-mobile').textContent = 'Erro ao carregar cotações';
    }
}
...
searchInput.addEventListener('input', debounce((e) => {
    ...
    searchResults.innerHTML = results.map(news => `
        <div class="search-result-item" onclick="window.open('${news.link}', '_blank')" style="background: white; margin-bottom: 0.5rem; padding: 1rem; border-radius: 8px; cursor: pointer;">
...
document.querySelectorAll('.language-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // Aqui você pode implementar traduções automáticas
    });
});
